...
class Settings(BaseSettings):
    ...
    ollama_url: str = Field(
        default="http://ollama.rag-app.svc.cluster.local:11434", alias="OLLAMA_URL"
    )
    ...
    milvus_address: str = Field(
        default="milvus.milvus.svc.cluster.local:19530", alias="MILVUS_ADDRESS"
    )
    milvus_collection: str = Field(default="rag_knowledge", alias="MILVUS_COLLECTION")
    knowledge_base_path: str = Field(
        default="/knowledge-base", alias="KNOWLEDGE_BASE_PATH"
    )
    auto_import_knowledge: bool = Field(default=False, alias="AUTO_IMPORT_KNOWLEDGE")

    retrieval_top_k: int = Field(default=5, alias="RETRIEVAL_TOP_K")
    retrieval_score_threshold: float = Field(
        default=0.5, alias="RETRIEVAL_SCORE_THRESHOLD"
    )
