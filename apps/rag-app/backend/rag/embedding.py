...
class EmbeddingClient:
    ...
    async def _embed_openai_compatible(self, text: str) -> list[float]:
        ...
        headers = self._auth_headers(self.settings.embedding_api_key or self.settings.llm_api_key)
    ...
    def _local_embedding(self, text: str) -> list[float]:
        ...
        vector = [((digest[index % len(digest)] / 255.0) * 2.0) - 1.0 for index in range(384)]
